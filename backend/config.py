...
def _load_backend_env() -> None:
    """Load backend/.env independently from the process working directory."""
    ...
    if load_dotenv is None or dotenv_values is None:
        if env_exists:
            raise RuntimeError(
                "python-dotenv is required to load backend/.env. "
    ...
    # Empty shell variables should not mask real values from backend/.env.
...
def _coerce_int(value: str | None, default: int, key: str) -> int:
    try:
        ...
        raise RuntimeError(f"Environment variable {key} must be a valid integer") from None
...
def get_db_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "port": _coerce_int(os.environ.get("DB_PORT"), 3306, "DB_PORT"),
    }
